...
function redesign() {
    ...
    $("#tdCorrection1stBox .ul").each(function () {
        ...
        var correct_word = $("li:nth-child(1) a", correct_item).text();
...
function check_same(id, check_word) {
    ...
    $(".ul").each(function () {
        ...
        //var errText = check_word.substr(check_word.length - repLen - 1,check_word.length);
...
function fRenew() {
    //alert("fRenew");
    //window.location.assign("http://speller.cs.pusan.ac.kr");
	setTimeout(function(){document.location.href = "http://speller.cs.pusan.ac.kr"},500);
}
...
//	document.body.appendChild(ifr);
    ...
    /*
        if((agent.indexOf("chrome") != -1)||(agent.indexOf("safari") != -1)||(agent.indexOf("Firefox") != -1)){
            ...
            document.getElementById('formBugReport1').action = 'clickReplace.asp';
        }

        else if ( (navigator.appName == 'Netscape' && agent.indexOf('trident') != -1) || (agent.indexOf("msie") != -1) ){
            //alert("익스플로러");
            document.charset = 'UTF-8';
            document.getElementById('formBugReport1').action = 'IEclickReplace.asp';
        }
    */
    if ((navigator.appName == 'Netscape' && agent.indexOf('trident') != -1) || (agent.indexOf("msie") != -1)) {
        //alert("익스플로러");
        document.charset = 'UTF-8';
        document.getElementById('formBugReport1').action = 'IEBugReporting.asp';
    }
    else {
        //alert("크롬");
        document.charset = 'EUC-KR';
        document.getElementById('formBugReport1').action = 'BugReporting.asp';
    }
	...
	if(document.forms['formBugReport1'].hiddenComment.value ==""){
		alert("내용을 입력해주세요.");
	} else {
...
function disableErrReportButton(index) {
	...
	if(index==null || index=="" || index=="skfkdlsvh")
	{
...
function fMakeButton() {
    document.getElementById('formNext').innerHTML
        = "<!--다음 검사할 내용-->"
    ;
    var formNextHtml = "<input type='hidden' id='text2' name='text2' value='empty'/>\n"
        + "<input type='hidden' id='oldPageNumber' name='pageNumber' value='"
        + document.getElementById('pageNumber').value
        + "'/>\n";
    $("#formNext").html(formNextHtml);
    //alert(formNextHtml);
    //document.getElementById('formNext').innerHTML
    ...
    var t = fGetPrevNextButtonHTML();
    $("#idButtons").html(t);
    //document.getElementById('idButtons').innerHTML = t;				// 하단 버튼
...
function fAddClickEventForUserInput(prefixIndex) {
    for (var i = 0; i < 1000; i++) {
        $('#btnShowUserInput_' + prefixIndex + i)
			...
			var btn = $('#btnShowUserInput_' + prefixIndex + i);
        ...
        $('#btnHideUserInput_' + prefixIndex + i)
			...
			btn = $('#btnHideUserInput_' + prefixIndex + i)
			btn.prop("disabled", true);
			btn.hide();
...
function fAddChangeEventForUserInput() {
    $('.inputCandidate')
        .bind("change paste", function () {
            //alert($(this).val());
            //$(this).val('xx')
            //console.log("클릭123123");
    ...
    (
        function (index, item) {
			...
			if ('tdReplace' == $(item).attr('class')) {
                ...
                newCandForm += "<LI class='liUserInputCandidate'><A href='#link'><input class='inputCandidate' type='text' placeholder='원하는 대치어를 직접 입력하세요.' onfocus='this.placeholder = ''' onblur='this.placeholder = '원하는 대치어를 직접 입력하세요.'' style='display: inline-block ;' value='";
                //newCandForm += $('#tdErrorWord_' + strDigitID).text();
                //console.log("직접입력 : " + $('#tdErrorWord_'+strDigitID).text());
                newCandForm += "'/><input type=text' style='display: none;' /></A></LI>"; //익스플로러에서 직접교정 안되는 것을 해결하기 위한 꼼수.
            }
...
/*
            if ('tdReplace' == $(item).attr('class')) {
                ...
                newCandForm += "<LI class='liUserInputCandidate'><A href='#link'><input class='inputCandidate' type='text' placeholder='원하는 대치어를 직접 입력하세요.' style='display: inline-block ;' value='";
                //newCandForm += $('#tdErrorWord_' + strDigitID).text();
                //console.log("직접입력 : " + $('#tdErrorWord_'+strDigitID).text());
                newCandForm += "' onfocus='this.value = this.value;'/><input type=text' style='display: none;' /></A></LI>"; //익스플로러에서 직접교정 안되는 것을 해결하기 위한 꼼수.
            }
*/
            newCandForm += "</UL>";
...
function fApplyCandidate(obj) {
    ...
    //console.log("objTD[0].id.toString() : " + objTD[0].id.toString());

    var strDigitID = fGetStrDigitID(objTD[0].id.toString());
    var errText = $('#tdErrorWord_' + strDigitID).text();
    //console.log("에러단어 : " + errText);
    //console.log("대치어 : " + objLI.text());
    ...
    var strText = obj.text();
    if (strText.length == 0 && innerInputValue.length != 0) {
        strText = innerInputValue;
        //console.log("사용자 대치어 : " + strText);
        if (errText != strText) {
            if(errText !=" " || errorText !="  "){
				//console.log('사용자 대치어 클릭');
               writeReplaceWord(errText, strText);
            }
        }
    }
...
function writeReplaceWord(ErrWord, RepWord) {
    var agent = navigator.userAgent.toLowerCase();
    /*
        if((agent.indexOf("chrome") != -1)||(agent.indexOf("safari") != -1)||(agent.indexOf("Firefox") != -1)){
            //alert("크롬");
    ...
    */
    if ((navigator.appName == 'Netscape' && agent.indexOf('trident') != -1) || (agent.indexOf("msie") != -1)) {
        ...
        document.charset = 'UTF-8';
        //document.getElementById('formBugReport1').action = 'IEclickReplace.asp';
		$('#formBugReport1').attr('action', 'IEwriteReplace.asp');
    }
    else { //그외.
        //alert("크롬");
        document.charset = 'EUC-KR';
        //document.getElementById('formBugReport1').action = 'clickReplace.asp';
		$('#formBugReport1').attr('action', 'writeReplace.asp');
	}
	var text = $('#bufUnderline').text();
	var delTag = text.replace(/(<([^>]+)>)/ig, "");
	//console.log(delTag);

	var pos = delTag.indexOf(errWord);
	...
	cnt = 0;
	var endStart = pos + errWord.length;
	...
	if(sentence.length == 0){
        ...
        document.getElementById('formBugReport1').target = 'ifHidden';
        document.forms['formBugReport1'].hiddenInputStr.value = "skfkdlsvh";
        document.forms['formBugReport1'].hiddenErrorIdx.value = "skfkdlsvh";
        document.forms['formBugReport1'].hiddenErrorWord.value = ErrWord;
        document.forms['formBugReport1'].hiddenReplaceWord.value = RepWord;
        document.forms['formBugReport1'].hiddenComment.value = sentence;
        document.getElementById('formBugReport1').submit();
}

function clickReplace(errWord, repWord, wordList) {
    var agent = navigator.userAgent.toLowerCase();
    /*
        if((agent.indexOf("chrome") != -1)||(agent.indexOf("safari") != -1)||(agent.indexOf("Firefox") != -1)){
            //alert("크롬");
    ...
    */
    if ((navigator.appName == 'Netscape' && agent.indexOf('trident') != -1) || (agent.indexOf("msie") != -1)) {
        //alert("익스플로러");
        document.charset = 'UTF-8';
        document.getElementById('formBugReport1').action = 'IEclickWord.asp';
		//$('#formBugReport1').attr('action', 'IEclickWord.asp');
    }
    else { //그외.
        //alert("크롬");
        document.charset = 'EUC-KR';
        document.getElementById('formBugReport1').action = 'clickWord.asp';
		//$('#formBugReport1').attr('action', 'clickWord.asp');
	}
	var text = $('#bufUnderline').text();
	var delTag = text.replace(/(<([^>]+)>)/ig, "");
	//console.log(delTag);
	...
	if(repWord !="" || repWord !=" " || repWord != null){
		document.getElementById('formBugReport1').target = 'ifHidden';
		document.forms['formBugReport1'].hiddenInputStr.value = "skfkdlsvh";
		document.forms['formBugReport1'].hiddenErrorIdx.value = wordList;
		document.forms['formBugReport1'].hiddenErrorWord.value = errWord;
		document.forms['formBugReport1'].hiddenReplaceWord.value = repWord;
		document.forms['formBugReport1'].hiddenComment.value = sentence;
		document.getElementById('formBugReport1').submit();
...
function inputIncorrect(){
	...
	console.log(incorrectArrSize + " : " + lastArrSize);
	var jsonArr = new Array();
	
	//for(var [key, value] of incorrectArr){
    ...
    if ((navigator.appName == 'Netscape' && agent.indexOf('trident') != -1) || (agent.indexOf("msie") != -1)) {
        //alert("익스플로러");
        document.charset = 'UTF-8';
        document.getElementById('formBugReport1').action = 'IEincorrectList.asp';
    }
    else { //그외.
        //alert("크롬");
        document.charset = 'EUC-KR';
        document.getElementById('formBugReport1').action = 'incorrectList.asp';
	}
	
	//전체 갯수와 데이터 넘기기
	document.getElementById('formBugReport1').target = 'ifHidden';
	document.forms['formBugReport1'].hiddenInputStr.value = "";
	document.forms['formBugReport1'].hiddenErrorIdx.value = "";
	document.forms['formBugReport1'].hiddenErrorWord.value = jsonData;
	document.forms['formBugReport1'].hiddenReplaceWord.value = "";
	document.forms['formBugReport1'].hiddenComment.value = "";
	document.getElementById('formBugReport1').submit();	
	}
...
function noChange(errWord) {
    ...
    */
    if ((navigator.appName == 'Netscape' && agent.indexOf('trident') != -1) || (agent.indexOf("msie") != -1)) {
        //alert("익스플로러");
        document.charset = 'UTF-8';
        document.getElementById('formBugReport1').action = 'IEnoChange.asp';
		//$('#formBugReport1').attr('action', 'IEclickWord.asp');
    }
    else { //그외.
        //alert("크롬");
        document.charset = 'EUC-KR';
        document.getElementById('formBugReport1').action = 'noChange.asp';
		//$('#formBugReport1').attr('action', 'clickWord.asp');
	}
...
function fChangeUIMode() {
    if (getCookie('uiMode') == 'old') {
        ...
        $("script").filter(function () {
            return $(this).attr("src") === "../speller.js"
        }).remove();
        ...
        $("script").filter(function () {
            return $(this).attr("src") === "../speller(for NewUI).js"
        }).remove();
        $("link").filter(function () {
            return $(this).attr("href") === "../redesign.css"
        }).remove();
        $("link").filter(function () {
            return $(this).attr("href") === "../redesign_ie.css"
        }).remove();
...
function fShowUserReportPopup(strPath) {
    fShowFramePopup(0, 80, 850, 565, strPath + "UserReport.html");
}
